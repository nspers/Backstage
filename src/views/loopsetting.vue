  ...
  <div class="material subMain">
      ...
      <div class="material-left">
        ...
        <div class="companyInfoBox">
          <ul class="companyInfoList">
            <li
              class="companyItem"
              :class="(item.gatewayId === nowGatewayId)?'ci_active':''"
              v-for="(item,index) in playDrectory"
              :key="index"
              @click="companyClick(item.gatewayId)"
            >{{item.name}}</li>
          </ul>
        </div>
      </div>
    ...
    <Modal v-model="addBox" width="400" :closable="false" @on-ok="addSend">
      <div class="f_box">
        ...
        <p style="margin-top: 20px;line-height: 30px;" v-if="!this.deviceList.length">无设备</p>
        <CheckboxGroup v-model="nowDev" style="margin-top: 20px;" v-else>
          <Checkbox
...
export default {
      ...
      columns12: [
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              "div",
              params.row.deviceName ? params.row.deviceName : "无名称"
            );
          }
        },
        {
  ...
  created: function() {
    //获取集控器列表
    this.$api.post("/device/findLoopGateway").then(res => {
      if (res.code === 1) {
  ...
  methods: {
      ...
      if (this.nowGatewayId) {
        if (this.nowLoop) {
          if (this.tableList.length > 0) {
            this.$Message.error("只能添加一个设备");
          } else {
            this.addBox = true;
          }
        } else {
          this.$Message.error("请选择回路");
        }
      } else {
        this.$Message.error("请选择集控器");
      }
    },
    companyClick(gwId) {
      this.nowLoop = 0;
    ...
    getLoopList(gwId, loopId) {
      ...
      this.$api.post(`device/findLoopListById/${gwId}/${loopId}`).then(res => {
        if (res.code === 1) {
    ...
    matchDevType(typecode) {
      var obj = this.deviceMenu.find(d => d.id == typecode);

      return obj ? obj.name : typecode;
    },
    //获取设备类型列表
    getDeviceMenu() {
      this.deviceMenu = [];
      ...
      this.deviceList = [];
      this.nowDev = [];
      this.$api.post("dictionaryDetails/getDeviceMenu").then(res => {
        if (res.code === 1) {
    ...
    getDeviceList(dtId) {
      this.deviceList = [];
      this.typeCode = dtId;
      this.$api.post("device/findDeviceByType/" + dtId).then(res => {
        if (res.code === 1) {
    ...
    addSend() {
      ...
      this.$api.post("device/addDevice/", sendData).then(res => {
        if (res.code === 1) {
    ...
    remove(row) {
      this.$Common.waring(() => {
        ...
        this.$api.post("device/removeDevice", sendData).then(res => {
          if (res.code === 1) {
            this.$Message.success(res.message);
            this.getLoopList(this.nowGatewayId, this.nowLoop);
          }
