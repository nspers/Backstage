<template>
  <div class="adbook subMain clearfix">
    ...
    <div class="top_btn clearfix">
      <span>报警设备</span>
      <Select v-model="pageTitle" style="width:200px;margin-left: 20px;" @on-change="smsTypeChange">
      ...
      <div class="f_box" v-if="which === 'add'||which === 'edit'">
        <Form :label-width="100" ref="formItems" :model="formItems">
          ...
          <FormItem
            ...
            :rules="{required: true, validator: $Validate.notEmpty, trigger: 'blur'}"
          >
            <Select v-model="formItems.receiverId" style="width:200px;" @on-change="receiverChange">
    ...
    <Tabs value="name1">
      <TabPane :label="pageTitle" name="name1">
...
export default {
  ...
  computed: {
    code: function() {
      if (this.alertEqtList.length) {
        return this.alertEqtList.filter(item => this.pageTitle == item.name)[0]
          .code;
      } else {
        return "-1";
      }
  ...
  created: function() {
    this.$api.post("dictionaryDetails/alarmType", {}).then(res => {
      if (res.code == 1) {
        this.alertEqtList = res.result;
        this.pageTitle = res.result[0].name;
        this.currentChange(1);
      }
    });
    this.$api
    ...
    poupSure(name) {
      ...
      if (this.which === "add") {
        this.$refs[name].validate(valid => {
          if (valid) {
            let data = {
              alarmEquipId: this.code,
              alarmEquipName: this.pageTitle,
              receiverId: 1,
