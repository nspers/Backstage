    ...
    >
      ...
      <div v-if="curStep === 0" class="abox_content">
        ...
        >
          ...
          <FormItem
            v-show="formItem1.controlMode == '1' || formItem1.controlMode == '2'"
...
var mixinApi = {
  ...
  // lcd
  "13": {
    find: "/strategy/findLcdStrategy",
    add: "/strategy/insertLcdStrategy",
    edit: "/strategy/updateLcdStrategy",
    del: "/strategy/deleteLcdStrategy/",
    tree: "/strategy/findLcdDid"
  }
};
mixinApi["22"] = mixinApi["10"] = mixinApi["11"] = mixinApi["9"];
export default {
  name: "App",
  data() {
    return {
            ...
            return h("i-switch", {
              ...
              on: {
                "on-change": value => {
                  ...
                  if (this.isType == "light") {
                    let loops = [];
                    params.row.gatewayLoopVoList.forEach(item => {
                      let obj = {
                        did: item.gatewayId,
                        gatewayDid: item.gatewayId,
                        loop: ""
                      };
                      let str = "";
                      if (item.loop) {
                        item.loop.forEach(ele => {
                          if (ele.number) {
                            str += !str ? ele.number : "," + ele.number;
                          }
                        });
                      }
                      obj["loop"] = str;
                    ...
                    this.param = {
                      ...
                      loops: loops,
                      ofSet: params.row.ofSet
                    };
                  } else if (this.isType == "led" || this.isType == "lcd") {
                    let taskDevice = [];
                  ...
                  this.$api
                    .post(mixinApi[this.deviceType].edit, this.param)
                    .then(res => {
                      if (res.code == 1) {
                        this.alertBox = false;
                        // this.loading = true;
                ...
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      ...
                      if (this.isType == "light") {
                        this.editTreeData = params.row.gatewayLoopVoList;
                        params.row.gatewayLoopVoList.forEach(item => {
  ...
  methods: {
    ...
    search_table() {
      ...
      this.$api.post(mixinApi[this.deviceType].find, param).then(res => {
        if (res.code == 1) {
    ...
    tree() {
      ...
      if (this.isType == "light") {
        this.$api
          .get(mixinApi[this.deviceType].tree + this.deviceType)
          .then(res => {
            if (res.code == 1) {
              ...
              // }
              // for (let i = 0; i < this.data2[0].children.length; i++) {
              //   for (
              //     let j = 0;
              //     j < this.data2[0].children[i].children.length;
              //     j++
              //   ) {
              //     this.data2[0].children[i].children[
              //       j
              //     ].title = this.data2[0].children[i].children[j].name;
              //     this.data2[0].children[i].children[j].checked = false;
              //   }
              // }

              // 2019-12-19 edit by cy
              for (let i = 0; i < list.length; i++) {
                this.data2[0].children.push({
                  title: list[i].name,
                  gatewayId: list[i].gatewayId,
                  checked: false,
                  children: list[i].loop.map(obj => {
                    return {
                      checked: false,
                      title: obj.name,
                      number: obj.number
        ...
        this.$api.get(mixinApi[this.deviceType].tree).then(res => {
          if (res.code == 1) {
            var list = res.result;
            for (let i = 0; i < list.length; i++) {
              this.object = {
      ...
      if (this.which == "add") {
        ...
        if (this.isType == "light") {
          ...
          this.treeData.power.forEach(item => {
            ...
            obj["did"] = item.gatewayId;
            obj["gatewayDid"] = item.gatewayId;
            let str = "";
            item.children.forEach(ele => {
              if (ele.checked) {
                str += !str ? ele.number : "," + ele.number;
        ...
        this.$api.post(mixinApi[this.deviceType].add, this.param).then(res => {
          if (res.code == 1) {
            this.alertBox = false;
            this.search_table();
            this.curStep = 0;
        ...
        this.$api.post(mixinApi[this.deviceType].edit, this.param).then(res => {
          if (res.code == 1) {
            this.alertBox = false;
            this.search_table();
            this.curStep = 0;
    ...
    // 删除
    delete(deleteid) {
      this.$api.post(mixinApi[this.deviceType].del + deleteid, {}).then(res => {
        if (res.code == 1) {
          this.search_table(); //初始化列表
        }
      });
